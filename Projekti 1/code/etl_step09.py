"""
Module Docstring:
A module to fetch all tables from a duckdb database and write the 
content of each table to a separate file.
"""

import os
import duckdb

class DuckDBExcavator:
    """
    Class Docstring:
...
if __name__ == "__main__":
    db_path = os.path.join('data', 'ultimate.duckdb')
    result_dir = os.path.join('data', 'results')
    extractor = DuckDBExcavator(db_path=db_path, result_dir=result_dir)
    extractor.process()
